...
import { LOG_BACKENDS } from '../constants'
...
/**
 * セキュリティログ表示コンポーネント
 * 選択されたバックエンドの、自サイト発信のログを表示
 */
export function LogViewer({ backendId }: Props) {
  // 自サイトのオリジンでフィルタリング
  const currentOrigin = window.location.origin
  const { data, isLoading, error, refetch } = useLogs(
    LOG_BACKENDS[backendId],
    backendId,
    100,
    currentOrigin
  )
  ...
  const getBackendLabel = (id: string) => {
    switch (id) {
      case 'A':
        return 'Backend A (同一オリジン)'
      case 'B':
        return 'Backend B (サブドメイン)'
      case 'C':
        return 'Backend C (別ドメイン)'
      default:
        return `Backend ${id}`
    }
  }
    ...
    <div className="bg-white rounded-lg p-5 mb-5 shadow border-l-4 border-red-600">
      <div className="flex items-center justify-between border-b border-gray-200 pb-2 mb-4">
        <h3 className="text-lg font-semibold text-gray-800 mt-0 mb-0">
          セキュリティログ - {getBackendLabel(backendId)}
        </h3>
        <button
          onClick={() => refetch()}
          disabled={isLoading}
          className="px-3 py-1.5 bg-red-600 text-white rounded text-sm hover:bg-red-700 disabled:opacity-60"
        >
          {isLoading ? '読込中...' : '更新'}
        </button>
      </div>
